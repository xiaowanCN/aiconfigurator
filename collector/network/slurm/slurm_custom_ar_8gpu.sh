...
# Multi-node custom allreduce sweep: TP=8 across 2 nodes (GB200/GB300: 4 GPUs/node,
# so TP>4 always spans nodes). Env template mirrors slurm_nccl_test_2node8gpu.sh;
# adjust NCCL_IB_HCA / NCCL_SOCKET_IFNAME / container-image to the cluster.
#SBATCH -N 2
#SBATCH --gpus-per-node=4
#SBATCH --ntasks-per-node=4
#SBATCH -o log_slurm_py/trtllm-bench-8gpu.out
#SBATCH -e log_slurm_py/trtllm-bench-8gpu.err
#SBATCH -J slurm_py_8gpu
